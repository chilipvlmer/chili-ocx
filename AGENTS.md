...
**Deploy:** Cloudflare Pages (automatic builds on push)
...
OCX supports several component types:
...
```bash
git add .
git commit -m "Add my-new-skill component"
git push
...
1. Verify the build completed successfully in Cloudflare Pages
2. Check the component appears in `dist/index.json`
3. Clear CDN cache if using one (Cloudflare auto-purges)
